...
public:
    ...
    virtual void    DoSomething(pfGUIControlMod* ctrl) {
        if (ctrl->GetTagID() == kKITempID_BarKIButtons) {
            ...
            if (fKIButtons->GetValue() == 0) {
                fKIMiniDlg->Show();
            }     else {
                fKIMiniDlg->Hide();
            }

            if (fKIButtons->GetValue() == 1) {
                fKIMainDlg->Show();
            }     else {
                fKIMainDlg->Hide();
            }
...
public:
    ...
    virtual const char*  GetText(void) {
        return fDataItem->GetText();
    }
    virtual void        SetText(const char* text) {
        fDataItem->SetText(text);
    }

    plKITextNoteElement* GetSource(void) {
        return fDataItem;
    }
};
...
public:
    ...
    void    SetLocalClientAsAdmin(bool yes) {
        fLocalClientIsAdmin = yes;
    }
    ...
    void    SetFadeOutDelay(hsScalar secs) {
        fFadeOutDelay = secs;
    }
    ...
    plKIFolder*  GetChatFolder(void) const {
        return fChatVaultFolder;
    }
