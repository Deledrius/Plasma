...
PYTHON_METHOD_DEFINITION(ptGUIControlListBox, addStringW, args)
{
    ...
    return PyLong_FromLong(self->fThis->AddString(plString::FromWchar(text)));
}
...
PYTHON_METHOD_DEFINITION(ptGUIControlListBox, findStringW, args)
{
    ...
    return PyLong_FromLong(self->fThis->FindString(plString::FromWchar(text)));
}
...
PYTHON_METHOD_DEFINITION(ptGUIControlListBox, addBranchW, args)
{
    ...
    else if (PyUnicode_Check(textObj))
    {
        ...
        const char* name = PyUnicode_AS_DATA(textObj);
        self->fThis->AddBranch(name, initiallyOpen != 0);
        PYTHON_RETURN_NONE;
    }
    else
