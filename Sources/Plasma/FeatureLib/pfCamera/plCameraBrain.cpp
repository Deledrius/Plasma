...
void plCameraBrain1::IMoveTowardGoal(double elapsedTime)
{
    ...
    } else if (plVirtualCam1::Instance()->fUseAccelOverride) {
    ...
    } else if (fFlags.IsBitSet(kPanicVelocity)) {
        IAdjustVelocity(fAccel, fDecel, &dir, &vel, 1000.0f, distToGoal, elapsedTime);
    } else if (fFlags.IsBitSet(kRunning) && fVelocity < 16.0f) { // speed up when we run if necessary
    ...
    } else if (fFlags.IsBitSet(kFalling)) {
        distMoved = IClampVelocity(&vel, plCameraBrain1::fFallVelocity, elapsedTime);
    } else if (fFlags.IsBitSet(kRunning) && fVelocity < 16.0f) {
...
void plCameraBrain1::IPointTowardGoal(double elapsedTime)
{
    ...
    } else if (fFlags.IsBitSet(kRunning) && fPOAVelocity < 16.0f) {
...
bool plCameraBrain1::MsgReceive(plMessage* msg)
{
    ...
    if (pCamMsg) {
        if (pCamMsg->Cmd(plCameraMsg::kStartZoomIn)) {
            ...
            return true;
        } else if (pCamMsg->Cmd(plCameraMsg::kStartZoomOut)) {
            fFlags.SetBit(kAnimateFOV);
            fFOVGoal = fZoomMax;
            fFOVAnimRate = fZoomRate;
            return true;
        } else if (pCamMsg->Cmd(plCameraMsg::kStopZoom)) {
            fFlags.ClearBit(kAnimateFOV);
            return true;
        } else if (pCamMsg->Cmd(plCameraMsg::kNonPhysOn)) {
            fFlags.SetBit(kNonPhys);
            return true;
        } else if (pCamMsg->Cmd(plCameraMsg::kNonPhysOff)) {
    ...
    if (pRefMsg) {
        if (pRefMsg->fType == kSubject) {
            ...
            return true;
        } else if (pRefMsg->fType == kRailComponent) {
            if (pRefMsg->GetContext() & (plRefMsg::kOnCreate | plRefMsg::kOnRequest | plRefMsg::kOnReplace)) {
    ...
    if (pCMsg) {
        if (pCMsg->ControlActivated()) {
            ...
            if (pCMsg->GetControlCode() == B_CAMERA_ZOOM_IN && fFlags.IsBitSet(kZoomEnabled)) {
                ...
                return true;
            } else if (pCMsg->GetControlCode() == B_CAMERA_ZOOM_OUT && fFlags.IsBitSet(kZoomEnabled)) {
                fFlags.SetBit(kAnimateFOV);
                fFOVGoal = fZoomMax;
                fFOVAnimRate = fZoomRate;
                fFOVEndTime = hsTimer::GetSysSeconds() + 60;
                return true;
            } else if (pCMsg->GetControlCode() == B_CAMERA_RECENTER) {
            ...
            } else if ((pCMsg->GetControlCode() == B_CAMERA_ZOOM_IN || pCMsg->GetControlCode() == B_CAMERA_ZOOM_OUT)
...
bool plCameraBrain1_Drive::MsgReceive(plMessage* msg)
{
    ...
    if (pMouseMsg) {
        if (pMouseMsg->GetDX() > 0.4 || pMouseMsg->GetDX() < -0.4) {
            bDisregardY = true;
            return true;
        } else if (pMouseMsg->GetDY() > 0.4 || pMouseMsg->GetDY() < -0.4) {
            bDisregardX = true;
            return true;
        }
        ...
        } else if (pMouseMsg->GetDX() > 0) {
            SetMovementFlag(B_CAMERA_ROTATE_LEFT);
            deltaX = pMouseMsg->GetDX();
        } else if (pMouseMsg->GetDY() > 0) {
...
void plCameraBrain1_Avatar::CalculatePosition()
{
    ...
    if (bObscured) {
        ...
        if (newOff.MagnitudeSquared() <= 16.0f && actualOff.MagnitudeSquared() <= 16.0f && !fFaded) {
            ISendFadeMsg(true);
            fFaded = true;
        } else if (newOff.MagnitudeSquared() >= 16.0f && fFaded) {
            ISendFadeMsg(false);
            fFaded = false;
        }
    } else if (fFaded && !bObscured) {
        ISendFadeMsg(false);
        fFaded = false;
    }
}
...
bool plCameraBrain1_Avatar::MsgReceive(plMessage* msg)
{
    ...
    if (behNotifymsg) {
        ...
        } else if (behNotifymsg->fType == plHBehavior::kBehaviorTypeFall) {
...
bool plCameraBrain1_Circle::MsgReceive(plMessage* msg)
{
    ...
    if (pRefMsg) {
        if (pRefMsg->fType == kCircleTarget) {
            ...
            return true;
        } else if (pRefMsg->fType == kPOAObject) {
            if (pRefMsg->GetContext() & (plRefMsg::kOnCreate | plRefMsg::kOnRequest | plRefMsg::kOnReplace)) {
            ...
            return true;
        } else if (pRefMsg->fType == kSubject) {
            if (pRefMsg->GetContext() & (plRefMsg::kOnCreate | plRefMsg::kOnRequest | plRefMsg::kOnReplace)) {
                SetSubject((plSceneObject*)pRefMsg->GetRef());
            } else {
                SetSubject(nil);
