...
void plVirtualCam1::FirstPersonOverride()
{
    ...
    if (fFirstPersonOverride) {
        ...
        fRetainedFY = fY;
        fX = fY = 0.5f;
    } else if (HasFlags(kFirstPersonEnabled)) {
...
bool plVirtualCam1::MsgReceive(plMessage* msg)
{
    ...
    if (behNotifymsg) {
        if (behNotifymsg->fType == plHBehavior::kBehaviorTypeLinkIn && behNotifymsg->state == true) {
            ...
            } else if (fFirstPersonOverride == nil) {
                plAvatarInputInterface::GetInstance()->CameraInThirdPerson(true);
            }
        } else if (behNotifymsg->fType == plHBehavior::kBehaviorTypeLinkOut && behNotifymsg->state == true) {
            ClearFlags(kFirstPersonEnabled);
    ...
    if (pCtrlMsg) {
        ...
        if (pCtrlMsg->GetControlCode() == B_CONTROL_MOVE_FORWARD || pCtrlMsg->GetControlCode() == B_CONTROL_MOVE_BACKWARD) {
        ...
        } else if (pCtrlMsg->GetControlCode() == S_SET_FIRST_PERSON_MODE && pCtrlMsg->ControlActivated()) {
            ...
            FirstPersonOverride();
            return true;
        } else if (pCtrlMsg->GetControlCode() == B_CAMERA_RECENTER && pCtrlMsg->ControlActivated()) {
    ...
    if (pCam) {
        if (pCam->Cmd(plCameraMsg::kResetPanning)) {
            fX = fY = fRetainedFY = 0.5f;
            return true;
        } else if (pCam->Cmd(plCameraMsg::kUpdateCameras)) {
            IUpdate();
            return true;
        } else if (pCam->Cmd(plCameraMsg::kResetOnEnter)) {
            // don't send this message to the virtual camera!
            Reset(false);
            // this only happens when the player links into the world
            return true;
        } else if (pCam->Cmd(plCameraMsg::kResetOnExit)) {
            /*
            ...
            */
            ...
            return true;
        } else if (pCam->Cmd(plCameraMsg::kCreateNewDefaultCam)) {
            if (pCam->GetSubject() == plNetClientMgr::GetInstance()->GetLocalPlayer()) {
                CreateDefaultCamera(pCam->GetSubject());
            }

            return true;
        } else if (pCam->Cmd(plCameraMsg::kPythonOverridePop)) {
        ...
        } else if (pCam->Cmd(plCameraMsg::kPythonOverridePush) || pCam->Cmd(plCameraMsg::kPythonOverridePushCut)) {
            if (pCam->GetTriggerer() && pCam->GetTriggerer() != plNetClientApp::GetInstance()->GetLocalPlayerKey()) {
                return true;
            }
        ...
        } else if (pCam->Cmd(plCameraMsg::kPythonSetFirstPersonOverrideEnable)) {
        ...
        } else if (pCam->Cmd(plCameraMsg::kPythonUndoFirstPerson)) {
            if (!pCam->HasBCastFlag(plMessage::kNetNonLocal)) {
                // make sure it was locally sent
                if (HasFlags(kFirstPersonAtLinkOut)) {
...
#endif
        } else if (pCam->Cmd(plCameraMsg::kResponderUndoThirdPerson)) {
            if (HasFlags(kScriptsForced3rd) || HasFlags(kScriptsDisabled1st)) {
                return true;
            }
        ...
        } else if (pCam->Cmd(plCameraMsg::kRegionPushCamera)) {
...
void plVirtualCam1::PopCamera(plCameraModifier1* pCam)
{
    ...
    if (plCameraBrain1_FirstPerson::ConvertNoRef(pCam->GetBrain())) {
        FreezeOutput(2);
        UnFadeAvatarIn(1);
    } else if (plCameraBrain1_Drive::ConvertNoRef(pCam->GetBrain()) ||
               !GetCurrentStackCamera()) {
            ...
            if (GetCurrentStackCamera()->GetNumTrans()) {
                for (int i = 0; i < GetCurrentStackCamera()->GetNumTrans(); i++) {
                    if (GetCurrentStackCamera()->GetTrans(i)->fTransTo == pCam->GetKey()) {
                        ...
                        break;
                    } else if (GetCurrentStackCamera()->GetTrans(i)->fTransTo == nil) {
                        pTrans = GetCurrentStackCamera()->GetTrans(i);
                    }
                }
            }
...
void plVirtualCam1::IHandleCameraStatusLog(plCameraModifier1* pMod, int action)
{
    ...
    if (pBrain) {
        ...
        } else if (plCameraBrain1_Fixed::ConvertNoRef(pBrain)) {
        ...
        } else if (plCameraBrain1_FirstPerson::ConvertNoRef(pBrain)) {
            camLog->AddLineF("Brain type 1st Person");
        } else if (plCameraBrain1_Avatar::ConvertNoRef(pBrain)) {
