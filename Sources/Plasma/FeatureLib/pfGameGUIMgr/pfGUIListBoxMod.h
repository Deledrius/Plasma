...
protected:
    ...
    struct plSmallRect {
        ...
        plSmallRect& operator=(const int zero) {
...
public:
    ...
    int32_t   GetSelection(void) {
        return fSingleSelElement;
    }
    ...
    // Export only
    void    SetScrollCtrl(pfGUIValueCtrl* ctrl) {
        fScrollControl = ctrl;
    }
    void    SetSingleSelect(bool yes) {
        if (yes) {
            SetFlag(kSingleSelect);
        } else {
            ClearFlag(kSingleSelect);
        }
    }
};
