...
public:
    ...
    const plFileName& GetName() const {
        return fName;
    }
    const plFileName& GetServerPath() const {
        return fServerPath;
    }
    const plMD5Checksum& GetChecksum() const {
        return fChecksum;
    }
    uint32_t GetDiskSize() const {
        return fSize;
    }
    uint32_t GetDownloadSize() const {
        return hsCheckBits(fFlags, kFlagZipped) ? fZippedSize : fSize;
    }
...
public:
    ...
    void SetFormatVersion(uint32_t v) {
        fFormatVersion = v;
    }
    ...
    uint32_t GetNumFiles() const {
        return fFiles.GetCount();
    }
    const plManifestFile& GetFile(uint32_t i) const {
        return *fFiles[i];
    }
};
