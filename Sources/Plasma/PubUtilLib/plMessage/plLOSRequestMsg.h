...
public:
    ...
    void SetFrom(const hsPoint3& from) {
        fFrom = from;
    }
    void SetTo(const hsPoint3& to) {
        fTo = to;
    }

    void SetWorldKey(plKey world) {
        fWorldKey = world;
    }

    /** Determines which database we're testing against.
        See plSimDefs::plLOSDB for a list of valid databases. */
    void SetRequestType(plSimDefs::plLOSDB type) {
        fRequestType = type;
    }
    plSimDefs::plLOSDB GetRequestType() const {
        return fRequestType;
    }
    ...
    void SetCullDB(plSimDefs::plLOSDB db) {
        fCullDB = db;
    }
    plSimDefs::plLOSDB GetCullDB() {
        return fCullDB;
    }
