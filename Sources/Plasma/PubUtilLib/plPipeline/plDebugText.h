...
public:
    ...
    static plDebugText&  Instance(void) {
        return fInstance;
    }

    uint32_t CalcStringWidth(const char* string);
    uint32_t CalcStringWidth_TEMP(const plString& string) {
        return CalcStringWidth(string.c_str());
    }
    ...
    void    SetDrawOnTopMode(bool enable) {
        fDrawOnTopMode = enable;
    }
    ...
    void    SetManager(plDebugTextManager* m) {
        fManager = m;
    }

    void            SetFont(const char* face, uint16_t size) {
        hsStrncpy(fFontFace, face, sizeof(fFontFace));
        fFontSize = size;
    }
    const char*     GetFontFace(void) {
        return fFontFace;
    }
    uint16_t        GetFontSize(void) {
        return fFontSize;
    }
...
public:
    ...
    plDebugTextManager() {
        plDebugText::Instance().SetManager(this);
        fFont = nil;
    }
    ~plDebugTextManager();
