...
public:
    ...
    void            SetChannelSelect(ChannelSelect source) {
        fChannelSelect = (uint8_t)source;
    }
    virtual uint8_t   GetChannelSelect(void) const {
        return fChannelSelect;
    }
...
protected:
    ...
    virtual bool    IActuallyPlaying(void) {
        return fReallyPlaying;
    }
    virtual void    IActuallyPlay(void);
    virtual void    IFreeBuffers(void);
    virtual bool    IActuallyLoaded(void) {
        return (fDSoundBuffer != nil) ? true : false;
    }
