...
uint32_t plBSDiffBuffer::Patch(uint32_t oldLength, void* oldBuffer, uint32_t& newLength, void*& newBuffer)
{
    ...
    */
    ...
    if (memcmp(fPatchBuffer, "QSUFDIFF", 8) == 0) {
        version = 1;
    }

    if (memcmp(fPatchBuffer, "BSDIFF30", 8) == 0) {
        version = 1;
    }

    if (memcmp(fPatchBuffer, "BSDIFF40", 8) == 0) {
        version = 2;
    }
    ...
    if ((newBuffer = (void*)new unsigned char[newLength + 1]) == nil) {
        return(-1);
    }
...
void plBSDiffBuffer::IWriteUnsignedint8_t(uint32_t x, unsigned char* buf)
{
    ...
    buf[0] = (unsigned char)(y % 256);
    y -= buf[0];
    y = y / 256;
    buf[1] = (unsigned char)(y % 256);
    y -= buf[1];
    y = y / 256;
    buf[2] = (unsigned char)(y % 256);
    y -= buf[2];
    y = y / 256;
    buf[3] = (unsigned char)(y % 256);
    y -= buf[3];
    y = y / 256;
    buf[4] = (unsigned char)(y % 256);
    y -= buf[4];
    y = y / 256;
    buf[5] = (unsigned char)(y % 256);
    y -= buf[5];
    y = y / 256;
    buf[6] = (unsigned char)(y % 256);
    y -= buf[6];
    y = y / 256;
    buf[7] = (unsigned char)(y % 256);
