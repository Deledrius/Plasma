...
protected:
    ...
    void        ISetMessageQueue(hsTArray<plCtrlCmd*>* queue) {
        fMessageQueue = queue;
    }
    plKeyMap*    IGetControlMap(void) const {
        return fControlMap;
    }
    bool        IOwnsControlCode(ControlEventCode code);
    bool        IVerifyShiftKey(plKeyDef key, int index);

    bool    IHasKeyControlFlag(int f) const {
        return fKeyControlFlags.IsBitSet(f);
    }
    void    ISetKeyControlFlag(int f)       {
        fKeyControlFlags.SetBit(f);
    }
    void    IClearKeyControlFlag(int which) {
        fKeyControlFlags.ClearBit(which);
    }
    void    IDisableControl(int which)      {
        fDisabledControls.SetBit(which);
    }
    void    IEnableControl(int which)       {
        fDisabledControls.ClearBit(which);
    }
