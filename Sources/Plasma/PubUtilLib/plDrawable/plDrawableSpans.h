...
public:
    ...
    bool        IsMatrixOnly() const {
        return 0 != (fFlags & kMatrixOnly);
    }
    bool        DontTransform() const {
        return 0 != (fFlags & kDontTransformSpans);
    }
    ...
    void        SetCountAndZero(int c) {
        fIndices.SetCountAndZero(c);
    }
...
protected:
    ...
    void            ICheckSpanForSortable(uint32_t idx) {
        if (!(fSpans[idx]->fProps & plSpan::kPropFacesSortable)) {
            IMakeSpanSortable(idx);
        }
    }
...
public:
    ...
    virtual plDrawable& SetNativeProperty(int prop, bool on) {
        if (on) {
            fProps |= prop;
        } else {
            fProps &= ~prop;
    ...
    virtual bool GetNativeProperty(int prop) const {
        return (fProps & prop) ? true : false;
    }
    ...
    virtual plSpaceTree*    GetSpaceTree() const {
        if (!fSpaceTree) {
            IQuickSpaceTree();
        }
    ...
    virtual const plSpan*                GetSpan(uint32_t index) const {
        return fSpans[ index ];
    }
    virtual const plSpan*                GetSpan(uint32_t diIndex, uint32_t index) const {
        return fSpans[(*fDIIndices[ diIndex ])[ index ] ];
    }
    virtual uint32_t                     GetNumSpans(void) const {
        return fSpans.GetCount();
    }
    virtual const hsTArray<plSpan*>&    GetSpanArray(void) const {
        return fSpans;
    }

    hsMatrix44* GetMatrixPalette(int baseMatrix) const {
        return &fLocalToWorlds[baseMatrix];
    }
    const hsMatrix44& GetPaletteMatrix(int i) const {
        return fLocalToWorlds[i];
    }
    ...
    plGBufferGroup*                         GetBufferGroup(uint32_t i) const {
        return fGroups[i];
    }
    uint32_t                                GetNumBufferGroups() const {
        return fGroups.GetCount();
    }
    const hsTArray<plGeometrySpan*>&        GetSourceSpans() const {
        return fSourceSpans;
    }
    ...
    hsGMaterial*     GetMaterial(uint32_t index) const {
        return ((index == (uint32_t) - 1) ? nil : fMaterials[ index ]);
    }
    ...
    uint32_t GetSkinTime() const {
        return fSkinTime;
    }
    void SetSkinTime(uint32_t t) {
        fSkinTime = t;
    }
