...
uint32_t  plDrawableSpans::IXlateSpanProps(uint32_t props, bool xlateToSpan)
{
    ...
    if (xlateToSpan) {
        ...
        if (props & kPropNoDraw) {
            retProps |= plSpan::kPropNoDraw;
        }

        if (props & kPropSortFaces) {
            retProps |= plSpan::kPropFacesSortable;
        }
    } else {
        /// plSpan props to Drawable props
        if (props & plSpan::kPropNoDraw) {
            retProps |= kPropNoDraw;
        }

        if (props & plSpan::kPropFacesSortable) {
            retProps |= kPropSortFaces;
        }
    }
...
void    plDrawableSpans::Read(hsStream* s, hsResMgr* mgr)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        case kSpanTypeIcicle:
            fSpans.Append((plSpan*)&fIcicles[ j & kSpanIDMask ]);
            break;

        case kSpanTypeParticleSpan:
            fSpans.Append((plSpan*)&fParticleSpans[ j & kSpanIDMask ]);
            break;
        }
...
bool    plDrawableSpans::IConvertGeoSpanToVertexSpan(plGeometrySpan* geoSpan, plVertexSpan* span, int lod, plVertexSpan* instancedParent)
{
    ...
    case plGeometrySpan::kLiteMaterial:
        span->fProps |= plSpan::kLiteMaterial;
        break;

    case plGeometrySpan::kLiteVtxPreshaded:
        span->fProps |= plSpan::kLiteVtxPreshaded;
        break;

    case plGeometrySpan::kLiteVtxNonPreshaded:
        span->fProps |= plSpan::kLiteVtxNonPreshaded;
        break;
    }
...
//  hsAssert( fProps & kPropVolatile, "Trying to remove spans on a non-volatile drawable" );
    ...
    for (i = 0; i < spanIndices->GetCount(); i++) {
        ...
        case kSpanTypeIcicle:
            fIcicles.Remove(idxRemoving & kSpanIDMask);
            break;

        case kSpanTypeParticleSpan:
            fParticleSpans.Remove(idxRemoving & kSpanIDMask);
            break;
        }
