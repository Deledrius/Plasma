...
//  size += sizeof( DWORD ) * 2;            // diffuse + specular
    ...
    default:
        hsAssert(false, "Bad weight count in GetVertexSize()");
    }
...
void    plGeometrySpan::EndCreate(void)
{
    ...
    for (i = 0, tempPtr = fVertexData; i < fNumVerts; i++) {
        ...
        if (fFormat & kSkinWeightMask) {
            ...
            default:
                hsAssert(false, "Garbage for weight format");
                break;
            }
...
void    plGeometrySpan::ExtractWeights(uint32_t vIdx, float* weightArray, uint32_t* indices)
{
    ...
    switch (fFormat & kSkinWeightMask) {
    case kSkinNoWeights:
        hsAssert(false, "ExtractWeights() called on a span with no weights");
        break;
    ...
    default:
        hsAssert(false, "Bad number of weights in ExtractWeights()");
    }
