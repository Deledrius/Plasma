...
public:
    ...
    hsBounds3(const hsBounds3& pRHS) : fBounds3Flags(0) {
        Reset(&pRHS);
    }
...
public:
    ...
    void    SetCenter(const hsBounds3* b)   {
        hsBounds3 bb = *b;
        fCenter = bb.GetCenter();
        fCenterValid = true;
    }
    void    SetCenter(const hsBoundsOriented* b)    {
        fCenter = b->GetCenter();
        fCenterValid = true;
    }
...
protected:
    ...
    bool IAxisIsZero(uint32_t i) const {
        return (fExtFlags & (1 << (20 + i))) != 0;
    };
...
public:
    ...
    hsBounds3Ext(const hsBounds3Ext& pRHS) {
        Reset(&pRHS);
    }
    ...
    virtual hsPoint3* GetCorner(hsPoint3* c) const {
        *c = (fExtFlags & kAxisAligned ? fMins : fCorner);
        return c;
    }
    virtual void GetCorners(hsPoint3* b) const;
    virtual bool ClosestPoint(const hsPoint3& p, hsPoint3& inner, hsPoint3& outer) const;
...
public:
    ...
    hsHitInfoExt(const hsPoint3* ctr, const hsVector3& offset) {
        fRootCenter = ctr;
        fDelPos = offset;
    };
