...
public:
    ...
    hsBitVector(uint32_t which) : fBitVectors(nil), fNumBitVectors(0) {
        SetBit(which);
    }
    ...
    bool operator!=(const hsBitVector& other) const {
        return !(*this == other);
    }
    hsBitVector& operator=(const hsBitVector& other); // will wind up identical

    bool ClearBit(uint32_t which) {
        return SetBit(which, 0);    // returns previous state
    }
    ...
    friend inline int Overlap(const hsBitVector& lhs, const hsBitVector& rhs) {
        return lhs.Overlap(rhs);
    }
    bool Overlap(const hsBitVector& other) const;
    bool Empty() const;

    bool operator[](uint32_t which) const {
        return IsBitSet(which);
    }
    ...
    hsBitVector& Compact();
    hsBitVector& SetSize(uint32_t numBits) {
        ClearBit(numBits + 1);
        return *this;
    }
    uint32_t GetSize() {
        return fNumBitVectors << 5;
    }

    // integer level access
    uint32_t GetNumBitVectors() const {
        return fNumBitVectors;
    }
    uint32_t GetBitVector(int i) const {
        return fBitVectors[i];
    }
    void SetNumBitVectors(uint32_t n) {
        Reset();
        fNumBitVectors = n;
        fBitVectors = new uint32_t[n];
    }
    void SetBitVector(int i, uint32_t val) {
        fBitVectors[i] = val;
    }
