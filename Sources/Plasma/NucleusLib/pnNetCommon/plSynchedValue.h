...
public:
    ...
    void MakeDirty() {
        SetFlags(GetFlags() | kValueIsDirty);
    }
    void MakeClean() {
        SetFlags(GetFlags() & ~kValueIsDirty);
    }
    ...
    // save/load
    static void Save(plSynchedValueBase& obj, hsStream* stream, hsResMgr* mgr) {
        obj.ISaveOrLoad(true, stream, mgr);
    }
    static void Load(plSynchedValueBase& obj, hsStream* stream, hsResMgr* mgr) {
        obj.ISaveOrLoad(false, stream, mgr);
    }
};
...
public:
    ...
    plSynchedValue() {}
    plSynchedValue(const T& v) : plSynchedValueBase() {
        fValue = v;
    }
    plSynchedValue(const plSynchedValue<T>& pRHS) : plSynchedValueBase() {
        fValue = pRHS.GetValue();    // copy ctor
    }
    ...
    // equality
    bool32 operator==(const T& other) const   {
        return fValue == (T)other;
    }
    bool32 operator!=(const T& other) const   {
        return !(*this == other);
    }
...
#pragma warning( pop )
    ...
    const T& operator=(const plSynchedValue<T>& pRHS)  {
        return SetValue(pRHS.GetValue());
    }
    ...
    // for hsBitVector
    bool32 IsBitSet(uint32_t which) const {
        return fValue.IsBitSet(which);
    }
    ...
    bool32 ClearBit(uint32_t which) {
        if (fValue.IsBitSet(which)) {
            DirtyIfNecessary();
        }

        return fValue.ClearBit(which);
    }
    void Reset() {
        if (fValue.GetSize() != 0) {
            DirtyIfNecessary();
    ...
    bool32 ToggleBit(uint32_t which) {
        DirtyIfNecessary();
        return fValue.ToggleBit(which);
    }
    uint32_t GetSize() {
...
public:
    ...
    const T& Get(int i) const {
        return fValueList.Get(i);
    }
    void    Set(int i, const T& item)   {
        if (fValueList[i] != item) {
            DirtyIfNecessary();
        }
    ...
    void    Append(const T& item)   {
        fValueList.Append(item);
        DirtyIfNecessary();
    }
    T*      Insert(int index)   {
        fValueList.Insert(index);
        DirtyIfNecessary();
    }
    void    Remove(int index)   {
        fValueList.Remove(index);
        DirtyIfNecessary();
    }
    int     Count() const {
        return fValueList.Count();
    }
    int     GetCount() const {
        return Count();
    }
    void    Reset() {
        if (fValueList.GetCount() != 0) {
            DirtyIfNecessary();
        }

        fValueList.Reset();
    }
    void    SetCountAndZero(int count)  {
        if (count || GetCount()) {
            DirtyIfNecessary();
        }

        fValueList.SetCountAndZero(count);
    }
    void    SetCount(int count)         {
        ...
        fValueList.SetCount(count);
    }
    void    ExpandAndZero(int count)    {
        if (count || GetCount()) {
            DirtyIfNecessary();
        }

        fValueList.ExpandAndZero(count);
    }
    int     Find(const T& item) const   {
        return fValueList.Find(item);
    }
    T*      Push()  {
        DirtyIfNecessary();
        return fValueList.Push();
    }
    void    Push(const T& item) {
        DirtyIfNecessary();
        return fValueList.Push(item);
    }
    T   Pop()   {
        DirtyIfNecessary();
        return fValueList.Pop();
    }
    const T&    Peek() const {
        return fValue.Peek();
    }
    T*      DetachArray()   {
        DirtyIfNecessary();
        return fValueList.DetachArray();
    }
    T*      AcquireArray()  {
        DirtyIfNecessary();
        return fValueList.AcquireArray();
    }
};
