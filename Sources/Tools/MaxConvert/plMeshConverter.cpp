...
public:
    ...
    plMAXVertNormal() {
        ...
        fNext = nil;
        fInited = false;
        fNormal = Point3(0, 0, 0);
    }
    plMAXVertNormal(Point3& n, DWORD s) {
        fNext = nil;
        fInited = true;
        fNormal = n;
        fSmGroup = s;
...
bool    plMeshConverter::CreateSpans(plMaxNode* node, hsTArray<plGeometrySpan*>& spanArray, bool doPreshading)
{
    ...
    try {
        ...
        if (maxUVWSrc > plGeometrySpan::kMaxNumUVChannels - 1) {
            maxUVWSrc = plGeometrySpan::kMaxNumUVChannels - 1;
        }
    ...
    for (i = 0; i < fNumPoints; i++) {
        if (weights[i].fIndices) {
            for (j = 0; j < 4; j++) {
                ...
                } else if (weights[i].fWeights[j] > penWgt) {
