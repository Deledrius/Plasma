...
public:
    ...
    void SetPoint(const Point3& p, const Point3& n) {
        fPos = p;    // Must be world space coming in.
        fNorm = fPertNorm = n;
    }
    void SetPoint(const hsPoint3& p, const hsVector3& n) {
        fPos = Point3(p.fX, p.fY, p.fZ);    // Must be world space coming in.
        fNorm = fPertNorm = Point3(n.fX, n.fY, n.fZ);
    }

    virtual BOOL InMtlEditor() {
        return false;    // is this rendering the mtl editor sample sphere?
    }

    virtual LightDesc* Light(int n) {
        return nil;    // get the nth light.
    }

    virtual TimeValue CurTime() {
        return fCurrentTime;    // current time value
    }
    ...
    virtual Point3 Normal() {
        return fPertNorm;    // interpolated surface normal, in cameara coords: affected by SetNormal()
    }
    virtual void SetNormal(Point3 p) {
        fPertNorm = p;    // used for perturbing normal
    }
    virtual Point3 OrigNormal() {
        return fNorm;    // original surface normal: not affected by SetNormal();
    }
    virtual Point3 GNormal() {
        return fNorm;    // geometric (face) normal
    }

    virtual Point3 V() {
        return Point3(0.f, 1.f, 0.f);    // Unit view vector: from camera towards P
    }
    virtual void SetView(Point3 p) {  } // Set the view vector

    virtual Point3 ReflectVector() {
        return V();    // reflection vector
    }
    virtual Point3 RefractVector(float ior) {
        return V();    // refraction vector
    }

    virtual Point3 CamPos() {
        return Point3(0, 0, 0);    // camera position
    }

    virtual Point3 P() {
        return fPos;    // point to be shaded;
    }
    virtual Point3 DP() {
        return Point3(0, 0, 0);    // deriv of P, relative to pixel, for AA
    }

    virtual Point3 PObj() {
        return P();    // point in obj coords
    }
    virtual Point3 DPObj() {
        return DP();    // deriv of PObj, rel to pixel, for AA
    }
    virtual Box3 ObjectBox() {
        return fBounds;    // Object extents box in obj coords
    }
    virtual Point3 PObjRelBox();        // Point rel to obj box [-1 .. +1 ]
    virtual Point3 DPObjRelBox() {
        return Point3(0, 0, 0);    // deriv of Point rel to obj box [-1 .. +1 ]
    }
    virtual void ScreenUV(Point2& uv, Point2& duv) {
        uv.Set(0, 0);    // screen relative uv (from lower left)
        duv.Set(0, 0);
    }
    virtual IPoint2 ScreenCoord() {
        return IPoint2(0, 0);    // integer screen coordinate (from upper left)
    }

    virtual Point3 UVW(int channel = 0) {
        return Point3(0, 0, 0);    // return UVW coords for point
    }
    virtual Point3 DUVW(int channel = 0) {
        return Point3(0, 0, 0);    // return UVW derivs for point
    }
    virtual void DPdUVW(Point3 dP[3], int channel = 0) {
        dP[0] = dP[1] = dP[2] = Point3(0, 0, 0);    // Bump vectors for UVW (camera space)
    }

    virtual void GetBGColor(Color& bgcol, Color& transp, BOOL fogBG = TRUE) {
        bgcol.Black();    // returns Background color, bg transparency
        transp.Black();
    }
